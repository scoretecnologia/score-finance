...
REPO_URL="https://github.com/score-finance-finance/score-finance.git"
...
setup_repo() {
  ...
  info "Cloning Score Finance repository..."
  git clone "$REPO_URL" score-finance
  cd score-finance
...
# ── Health Check ─────────────────────────────────────────────────────────────
wait_for_health() {
  info "Waiting for Score Finance to be ready (up to ${HEALTH_TIMEOUT}s)..."
  local elapsed=0

  while [ "$elapsed" -lt "$HEALTH_TIMEOUT" ]; do
    if curl -sf "$HEALTH_URL" > /dev/null 2>&1; then
      success "Score Finance is healthy"
      return
    fi
    sleep 3
    elapsed=$((elapsed + 3))
...
main() {
  echo ""
  echo -e "${BOLD}╔══════════════════════════════════════╗${NC}"
  echo -e "${BOLD}║         Score Finance Installer             ║${NC}"
  echo -e "${BOLD}╚══════════════════════════════════════╝${NC}"
  echo ""
  ...
  echo -e "${GREEN}${BOLD}  Score Finance is running!${NC}"
  ...
  echo -e "    ${BLUE}docker compose -f $COMPOSE_FILE down${NC}       # Stop Score Finance"
  echo ""
}
