...
async def create_connect_token(
    ...
    try:
        token_data = await connection_service.create_connect_token(data.provider, company.id)
...
async def list_connections(
    ...
    return await connection_service.get_connections(session, company.id)
...
async def get_oauth_url(
    ...
    try:
        url = connection_service.get_oauth_url(data.provider, company.id)
...
async def oauth_callback(
    ...
    try:
        connection = await connection_service.handle_oauth_callback(
            session, company.id, user.id, data.code, data.provider
        )
        return connection
    except Exception as e:
        raise HTTPException(
...
async def sync_connection(
    ...
    try:
        connection, merged_count = await connection_service.sync_connection(session, connection_id, company.id)
...
async def get_reconnect_token(
    ...
    connection = await connection_service.get_connection(session, connection_id, company.id)
    if not connection:
    ...
    try:
        token_data = await connection_service.create_connect_token(
            connection.provider, company.id, item_id=item_id
        )
...
async def update_settings(
    ...
    connection = await connection_service.update_connection_settings(
        session, connection_id, company.id, data.model_dump(exclude_unset=True)
    )
    if not connection:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Connection not found")
    return connection
...
async def delete_connection(
    ...
    deleted = await connection_service.delete_connection(session, connection_id, company.id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Connection not found")
...
async def detect_transfers(
    ...
    pairs_created = await detect_transfer_pairs(session, company.id)
...
async def unlink_transfer(
    ...
    unlinked = await unlink_transfer_pair(session, company.id, pair_id)
    if not unlinked:
