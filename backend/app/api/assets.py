...
async def list_assets(
    ...
    assets = await asset_service.get_assets(session, company.id, include_archived=include_archived)
...
async def portfolio_trend(
    ...
    return await asset_service.get_portfolio_trend(session, company.id)
...
async def get_asset(
    ...
    asset = await asset_service.get_asset(session, asset_id, company.id)
    if not asset:
...
async def create_asset(
    ...
    return await asset_service.create_asset(session, company.id, data)
...
async def update_asset(
    ...
    asset = await asset_service.update_asset(session, asset_id, company.id, data, regenerate_growth=regenerate_growth)
    if not asset:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Asset not found")
    return asset
...
async def delete_asset(
    ...
    deleted = await asset_service.delete_asset(session, asset_id, company.id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Asset not found")
...
async def list_asset_values(
    ...
    values = await asset_service.get_asset_values(session, asset_id, company.id)
    if values is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Asset not found")
    return values
...
async def get_asset_value_trend(
    ...
    trend = await asset_service.get_asset_value_trend(session, asset_id, company.id, months=months)
    if trend is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Asset not found")
    return trend
...
async def add_asset_value(
    ...
    value = await asset_service.add_asset_value(session, asset_id, company.id, data)
    if value is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Asset not found")
    return value
...
async def delete_asset_value(
    ...
    deleted = await asset_service.delete_asset_value(session, value_id, company.id)
    if not deleted:
