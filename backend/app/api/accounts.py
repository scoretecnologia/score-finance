...
async def list_accounts(
    ...
    accounts = await account_service.get_accounts(session, company.id, include_closed=include_closed)
...
async def get_account_summary(
    ...
    summary = await account_service.get_account_summary(
        session, account_id, company.id, date_from=from_date, date_to=to_date,
    )
    if not summary:
...
async def delete_account(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Could not delete account. It may have related records.")
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Account not found")
