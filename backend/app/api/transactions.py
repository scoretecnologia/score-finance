...
async def list_transactions(
    ...
    chart_account_id: Optional[uuid.UUID] = Query(None),
    chart_account_ids: Optional[List[uuid.UUID]] = Query(None),
...
async def create_transfer(
    ...
    try:
        ...
        debit_full = await transaction_service.get_transaction(session, debit_tx.id, company.id)
        credit_full = await transaction_service.get_transaction(session, credit_tx.id, company.id)
        primary_currency = user.primary_currency
        return TransferRead(
...
async def link_transfer(
    ...
    try:
        debit_tx, credit_tx = await transaction_service.link_existing_as_transfer(
            session, company.id, data.transaction_ids
        )
        debit_full = await transaction_service.get_transaction(session, debit_tx.id, company.id)
        credit_full = await transaction_service.get_transaction(session, credit_tx.id, company.id)
        primary_currency = user.primary_currency
...
async def get_transfer_candidates(
    ...
    anchor = await transaction_service.get_transaction(session, transaction_id, company.id)
    if not anchor:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Transaction not found")
    candidates = await transaction_service.get_transfer_candidates(
        session, company.id, transaction_id, limit=limit, window_days=window_days
    )
    primary_currency = user.primary_currency
    return [
...
async def get_transaction(
    ...
    transaction = await transaction_service.get_transaction(session, transaction_id, company.id)
    if not transaction:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Transaction not found")
    primary_currency = user.primary_currency
