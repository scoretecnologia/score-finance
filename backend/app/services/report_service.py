...
async def _net_worth_at(
    ...
    assets_total = await _asset_value_at(session, company_id, cutoff, primary_currency)
...
async def get_net_worth_report(
    ...
    for point in points:
        dp = await _net_worth_at(session, company_id, point, primary_currency)
...
async def get_income_expenses_report(
    ...
    while cursor <= today:
        ...
        projections = await _get_recurring_projections(session, company_id, m_start, m_end)
