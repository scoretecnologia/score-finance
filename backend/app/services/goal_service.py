...
async def _get_primary_currency(session: AsyncSession, company_id: uuid.UUID) -> str:
...
async def _resolve_current_amount(
    session: AsyncSession, goal: Goal, company_id: uuid.UUID
) -> Decimal:
    ...
    if goal.tracking_type == "account" and goal.account_id:
        ...
        # Add asset values
        assets_by_currency = await get_total_asset_value(session, company_id)
...
async def _enrich_goal(
    ...
    current = await _resolve_current_amount(session, goal, company_id)
    ...
    primary_currency = await _get_primary_currency(session, company_id)
    target_primary = None
...
async def get_goals(
    session: AsyncSession, company_id: uuid.UUID, status: Optional[str] = None
) -> list[GoalRead]:
    query = select(Goal).where(Goal.company_id == company_id).order_by(Goal.position, Goal.created_at)
    if status:
        query = query.where(Goal.status == status)
    result = await session.execute(query)
    goals = list(result.scalars().all())
    return [await _enrich_goal(session, g, company_id) for g in goals]


async def get_goal(
    session: AsyncSession, goal_id: uuid.UUID, company_id: uuid.UUID
) -> Optional[GoalRead]:
    result = await session.execute(
        select(Goal).where(Goal.id == goal_id, Goal.company_id == company_id)
    )
    goal = result.scalar_one_or_none()
    if not goal:
        return None
    return await _enrich_goal(session, goal, company_id)


async def create_goal(
    session: AsyncSession, company_id: uuid.UUID, data: GoalCreate
) -> GoalRead:
    goal = Goal(
    ...
    initial = await _resolve_current_amount(session, goal, company_id)
...
async def update_goal(
    session: AsyncSession, goal_id: uuid.UUID, company_id: uuid.UUID, data: GoalUpdate
) -> Optional[GoalRead]:
    result = await session.execute(
        select(Goal).where(Goal.id == goal_id, Goal.company_id == company_id)
    )
...
async def get_goal_summary(
    session: AsyncSession, company_id: uuid.UUID, limit: int = 3
) -> list[GoalSummary]:
