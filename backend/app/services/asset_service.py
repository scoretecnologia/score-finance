...
async def get_asset(
    session: AsyncSession, asset_id: uuid.UUID, company_id: uuid.UUID
) -> Optional[AssetRead]:
...
async def create_asset(
    session: AsyncSession, company_id: uuid.UUID, data: AssetCreate
) -> AssetRead:
...
async def update_asset(
    session: AsyncSession, asset_id: uuid.UUID, company_id: uuid.UUID, data: AssetUpdate,
...
async def get_asset_values(
    ...
    owner_check = await session.execute(
        select(Asset.id).where(Asset.id == asset_id, Asset.company_id == company_id)
    )
...
async def add_asset_value(
    session: AsyncSession, asset_id: uuid.UUID, company_id: uuid.UUID, data: AssetValueCreate
...
async def delete_asset_value(
    session: AsyncSession, value_id: uuid.UUID, company_id: uuid.UUID
) -> bool:
    """Delete a specific asset value entry."""
    result = await session.execute(
        select(AssetValue)
        .join(Asset, AssetValue.asset_id == Asset.id)
        .where(AssetValue.id == value_id, Asset.company_id == company_id)
    )
...
async def get_asset_value_trend(
    session: AsyncSession, asset_id: uuid.UUID, company_id: uuid.UUID, months: int = 12
) -> Optional[list[dict]]:
