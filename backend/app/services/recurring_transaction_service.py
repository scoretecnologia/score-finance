...
async def _verify_account_owned(
    session: AsyncSession, company_id: uuid.UUID, account_id: uuid.UUID
) -> None:
...
async def get_recurring_transactions(
    ...
    result = await session.execute(
        ...
        .where(RecurringTransaction.company_id == company_id)
...
async def get_recurring_transaction(
    session: AsyncSession, recurring_id: uuid.UUID, company_id: uuid.UUID
) -> Optional[RecurringTransaction]:
    result = await session.execute(
        select(RecurringTransaction)
        .where(RecurringTransaction.id == recurring_id, RecurringTransaction.company_id == company_id)
    )
    return result.scalar_one_or_none()


async def create_recurring_transaction(
    session: AsyncSession, company_id: uuid.UUID, data: RecurringTransactionCreate
) -> RecurringTransaction:
    await _verify_account_owned(session, company_id, data.account_id)
...
async def update_recurring_transaction(
    session: AsyncSession, recurring_id: uuid.UUID, company_id: uuid.UUID, data: RecurringTransactionUpdate
) -> Optional[RecurringTransaction]:
    recurring = await get_recurring_transaction(session, recurring_id, company_id)
    if not recurring:
        return None
    ...
    if "account_id" in update_data:
        ...
        if new_account_id != recurring.account_id:
            await _verify_account_owned(session, company_id, new_account_id)
...
async def generate_pending(
    session: AsyncSession, company_id: uuid.UUID, up_to: Optional[date] = None
) -> int:
