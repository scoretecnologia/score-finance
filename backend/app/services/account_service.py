...
async def get_accounts(session: AsyncSession, company_id: uuid.UUID, include_closed: bool = False) -> list[dict]:
...
async def get_account(session: AsyncSession, account_id: uuid.UUID, company_id: uuid.UUID) -> Optional[Account]:
...
async def create_account(session: AsyncSession, company_id: uuid.UUID, data: AccountCreate) -> Account:
...
async def update_account(
    session: AsyncSession, account_id: uuid.UUID, company_id: uuid.UUID, data: AccountUpdate
) -> Optional[Account]:
    account = await get_account(session, account_id, company_id)
    if not account:
        return None
...
async def delete_account(session: AsyncSession, account_id: uuid.UUID, company_id: uuid.UUID) -> bool:
    account = await get_account(session, account_id, company_id)
    if not account:
        return False
