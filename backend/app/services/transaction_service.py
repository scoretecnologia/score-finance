...
async def get_transaction(
    session: AsyncSession, transaction_id: uuid.UUID, company_id: uuid.UUID
...
async def create_transaction(
    session: AsyncSession, company_id: uuid.UUID, data: TransactionCreate
) -> Transaction:
...
async def create_transfer(
    session: AsyncSession, company_id: uuid.UUID, data: TransferCreate
    ...
    await stamp_primary_amount(session, company_id, debit_tx)
    await stamp_primary_amount(session, company_id, credit_tx)
...
async def get_transfer_candidates(
    ...
    anchor = await get_transaction(session, transaction_id, company_id)
    if not anchor:
        return []
...
async def link_existing_as_transfer(
    session: AsyncSession, company_id: uuid.UUID, transaction_ids: list[uuid.UUID]
) -> tuple[Transaction, Transaction]:
...
async def update_transaction(
    session: AsyncSession, transaction_id: uuid.UUID, company_id: uuid.UUID, data: TransactionUpdate
) -> Optional[Transaction]:
    transaction = await get_transaction(session, transaction_id, company_id)
    if not transaction:
        return None
    ...
    if transaction.transfer_pair_id and (cascade_fields & update_data.keys()):
        ...
        if paired_tx:
            ...
            await stamp_primary_amount(session, company_id, paired_tx)
