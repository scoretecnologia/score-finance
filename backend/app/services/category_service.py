...
async def create_default_categories(session: AsyncSession, company_id: uuid.UUID, lang: str = "pt-BR") -> list[Category]:
    # Guard against double-creation (race between categories and groups endpoints)
    existing = await session.execute(
        select(Category).where(Category.company_id == company_id).limit(1)
    )
    if existing.scalar_one_or_none():
        return await get_categories(session, company_id)

    # Create default groups first
    groups = await create_default_groups(session, company_id, lang)
...
async def create_category(session: AsyncSession, company_id: uuid.UUID, data: CategoryCreate) -> Category:
...
async def update_category(
    session: AsyncSession, category_id: uuid.UUID, company_id: uuid.UUID, data: CategoryUpdate
) -> Optional[Category]:
    category = await get_category(session, category_id, company_id)
    if not category:
        return None
...
async def delete_category(session: AsyncSession, category_id: uuid.UUID, company_id: uuid.UUID) -> bool:
    category = await get_category(session, category_id, company_id)
