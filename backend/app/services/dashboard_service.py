...
async def _get_recurring_projections(
    session: AsyncSession, company_id: uuid.UUID, month_start: date, month_end: date
) -> list[dict]:
...
async def get_summary(
    ...
    total_balance = await _total_balance_by_currency(session, company_id, cutoff)
    ...
    if month_end > cutoff:
        ...
        balance_projections = await _get_recurring_projections(
            session, company_id, projection_start, month_end
        )
    ...
    projections = await _get_recurring_projections(session, company_id, month_start, month_end)
    ...
    # Asset values
    assets_value = await get_total_asset_value(session, company_id)
...
async def get_monthly_trend(
    session: AsyncSession, company_id: uuid.UUID, months: int = 6
) -> list[MonthlyTrend]:
    accounting_mode = await get_credit_card_accounting_mode(session)
    report_date = (
        Transaction.effective_date if accounting_mode == "accrual" else Transaction.date
...
async def _get_open_accounts(
    ...
    result = await session.execute(
        ...
        .where(
            or_(Account.company_id == company_id, BankConnection.company_id == company_id),
            Account.is_closed == False,
        )
    )
...
async def _balance_at(
    ...
    totals = await _total_balance_by_currency(session, company_id, cutoff)
...
async def _daily_deltas(
    session: AsyncSession, company_id: uuid.UUID, start: date, end: date
) -> dict[int, float]:
...
async def get_balance_history(
    ...
    # Starting balances
    current_start = await _balance_at(session, company_id, month_start - timedelta(days=1))
    prev_start = await _balance_at(session, company_id, prev_month_start - timedelta(days=1))

    # Daily deltas from real transactions
    current_deltas = await _daily_deltas(session, company_id, month_start, month_end)
    prev_deltas = await _daily_deltas(session, company_id, prev_month_start, prev_month_end)
    ...
    if month_end > today:
        ...
        projections = await _get_recurring_projections(session, company_id, proj_start, month_end)
        for proj in projections:
            day = proj["date"].day
            proj_converted, _ = await convert(
                session, Decimal(str(proj["amount"])), proj["currency"], primary_currency,
