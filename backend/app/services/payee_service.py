...
async def get_payee(session: AsyncSession, payee_id: uuid.UUID, company_id: uuid.UUID) -> Optional[Payee]:
    result = await session.execute(
        select(Payee).where(Payee.id == payee_id, Payee.company_id == company_id)
    )
    return result.scalar_one_or_none()


async def get_or_create_payee(session: AsyncSession, company_id: uuid.UUID, name: str) -> Payee:
    ...
    result = await session.execute(
        select(Payee).where(Payee.company_id == company_id, func.lower(Payee.name) == name.lower())
    )
    payee = result.scalar_one_or_none()
    if payee:
        return payee

    payee = Payee(company_id=company_id, name=name)
    session.add(payee)
    await session.flush()
    return payee


async def create_payee(session: AsyncSession, company_id: uuid.UUID, data: PayeeCreate) -> Payee:
    # Check uniqueness
    existing = await session.execute(
        select(Payee).where(Payee.company_id == company_id, func.lower(Payee.name) == data.name.strip().lower())
    )
    if existing.scalar_one_or_none():
        raise ValueError("A payee with this name already exists")

    payee = Payee(company_id=company_id, **data.model_dump())
    session.add(payee)
    await session.flush()
...
async def update_payee(
    session: AsyncSession, payee_id: uuid.UUID, company_id: uuid.UUID, data: PayeeUpdate
) -> Optional[Payee]:
    payee = await get_payee(session, payee_id, company_id)
    if not payee:
        return None
...
async def delete_payee(session: AsyncSession, payee_id: uuid.UUID, company_id: uuid.UUID) -> bool:
    payee = await get_payee(session, payee_id, company_id)
    if not payee:
        return False
...
async def merge_payees(
    ...
    # Validate target
    target = await get_payee(session, target_id, company_id)
    if not target:
    ...
    for source_id in source_ids:
        if source_id == target_id:
            continue
        source = await get_payee(session, source_id, company_id)
        if not source:
