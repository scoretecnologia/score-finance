...
async def _match_pluggy_category(
    session: AsyncSession, company_id: uuid.UUID, pluggy_category: Optional[str]
) -> Optional[uuid.UUID]:
    ...
    result = await session.execute(
        select(Category.id).where(Category.company_id == company_id, Category.name == app_name)
    )
    return result.scalar_one_or_none()


async def get_connections(session: AsyncSession, company_id: uuid.UUID) -> list[BankConnection]:
    result = await session.execute(
...
async def get_connection(
    session: AsyncSession, connection_id: uuid.UUID, company_id: uuid.UUID
) -> Optional[BankConnection]:
    result = await session.execute(
        select(BankConnection)
        .where(BankConnection.id == connection_id, BankConnection.company_id == company_id)
        .options(selectinload(BankConnection.accounts))
    )
    return result.scalar_one_or_none()


def get_oauth_url(provider_name: str, company_id: uuid.UUID) -> str:
...
async def create_connect_token(
    provider_name: str, company_id: uuid.UUID, item_id: str | None = None
) -> dict:
    provider = get_provider(provider_name)
    token_data = await provider.create_connect_token(str(company_id), item_id=item_id)
...
async def update_connection_settings(
    ...
    connection = await get_connection(session, connection_id, company_id)
    if not connection:
        return None
...
async def handle_oauth_callback(
    session: AsyncSession, company_id: uuid.UUID, user_id: uuid.UUID, code: str, provider_name: str
) -> BankConnection:
    provider = get_provider(provider_name)
    ...
    for acc_data in connection_data.accounts:
        ...
        for txn_data in transactions_data:
            category_id = await _match_pluggy_category(
                session, company_id, txn_data.pluggy_category
            )
            # Resolve payee entity from raw payee text
            payee_id = None
            if txn_data.payee:
                payee_entity = await get_or_create_payee(session, company_id, txn_data.payee)
    ...
    await detect_transfer_pairs(session, company_id, candidate_ids=new_tx_ids)
    ...
    try:
        ...
        for acc_data in accounts_data:
            ...
            for txn_data in transactions_data:
                ...
                sync_payee_id = None
                if txn_data.payee:
                    sync_payee_entity = await get_or_create_payee(session, company_id, txn_data.payee)
