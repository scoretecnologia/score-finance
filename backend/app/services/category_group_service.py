...
async def create_default_groups(session: AsyncSession, company_id: uuid.UUID, lang: str = "pt-BR") -> dict[str, CategoryGroup]:
...
async def get_groups(session: AsyncSession, company_id: uuid.UUID) -> list[CategoryGroup]:
...
async def get_group(session: AsyncSession, group_id: uuid.UUID, company_id: uuid.UUID) -> Optional[CategoryGroup]:
    result = await session.execute(
        select(CategoryGroup)
...
async def create_group(session: AsyncSession, company_id: uuid.UUID, data: CategoryGroupCreate) -> CategoryGroup:
    group = CategoryGroup(company_id=company_id, **data.model_dump())
    session.add(group)
    await session.commit()
    return await get_group(session, group.id, company_id)


async def update_group(
    session: AsyncSession, group_id: uuid.UUID, company_id: uuid.UUID, data: CategoryGroupUpdate
) -> Optional[CategoryGroup]:
    group = await get_group(session, group_id, company_id)
    if not group:
        return None
    ...
    await session.commit()
    return await get_group(session, group_id, company_id)


async def delete_group(session: AsyncSession, group_id: uuid.UUID, company_id: uuid.UUID) -> bool:
    group = await get_group(session, group_id, company_id)
