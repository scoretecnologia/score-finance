...
async def _get_existing_rule_names(session: AsyncSession, company_id: uuid.UUID) -> set[str]:
    """Get the set of existing rule names for a user."""
    result = await session.execute(
        select(Rule.name).where(Rule.company_id == company_id)
    )
    return {row[0] for row in result.all()}


async def create_default_rules(session: AsyncSession, company_id: uuid.UUID, lang: str = "pt-BR") -> list[Rule]:
    """Create universal default categorization rules for a new user."""
    result = await session.execute(select(Category).where(Category.company_id == company_id))
...
async def install_rule_pack(session: AsyncSession, company_id: uuid.UUID, pack_code: str, lang: str = "pt-BR") -> list[Rule]:
...
async def get_installed_packs(session: AsyncSession, company_id: uuid.UUID) -> dict[str, bool]:
...
async def create_rule(session: AsyncSession, company_id: uuid.UUID, data: RuleCreate) -> Rule:
    existing_names = await _get_existing_rule_names(session, company_id)
...
async def update_rule(
    ...
    rule = await get_rule(session, rule_id, company_id)
    if not rule:
        return None
...
async def delete_rule(session: AsyncSession, rule_id: uuid.UUID, company_id: uuid.UUID) -> bool:
    rule = await get_rule(session, rule_id, company_id)
    if not rule:
        return False
